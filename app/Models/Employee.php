...
//    }
    ...
    public function hasChildren()
    {
        ...
        $this->request_types()->count() > 0 ||
        $this->plans()->count() > 0 ||
        $this->documents()->count() > 0||
        $this->tasksOwner()->count() > 0||
        $this->tasksSupervisor()->count() > 0||
        $this->managers()->count() > 0||
        $this->EmployeesTimeTablesDetails()->count() > 0||
        $this->fingerprints()->count() > 0||
        $this->managersData()->count() > 0||
        $this->documentHeader()->count() > 0||
        $this->user()->count() > 0||
        $this->internalSalesman()->count() > 0
