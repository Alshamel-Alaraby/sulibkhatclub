...
class RpBreakDown extends Model
{
    ...
    public function scopeData($query)
    {
        return $query
            ->select(
                'id', 'customer_id', 'client_type_id', 'break_id', 'break_type', 'debit', 'credit', 'instalment_date', 'total', 'document_id'
            )
            ->with([
                'documentHeader' => function ($q) {
                    $q->with([
                        'documentHeaderDetails' => function ($query) {
                            $query->with([
                                'realEstateUnit:id,name,name_e',
                            ]);
                        },
                    ]);
    ...
    {
        return $this->hasMany(\App\Models\Transaction::class, 'break_id');
    }
    ...
    public function hasChildren()
    {
        ...
        if ($this->children()->count() > 0) {
            $relationsWithChildren[] = [
                ...
                'ids' => $this->children()->pluck('instalment_date')->toArray(),
            ];
        }
        if ($this->breakSettlements()->count() > 0) {
            $relationsWithChildren[] = [
                'relation' => 'breakSettlements',
