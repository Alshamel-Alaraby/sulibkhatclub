    ...
    public function destroy($id)
    {
        ...
        if (!empty($relationsWithChildren)) {
            ...
            foreach ($relationsWithChildren as $relation) {
                ...
                $errorMessages[] = [
                    "message" => "This item has {$childCount} {$relationName} (rates : {$childIds}) and can't be deleted. Remove its {$relationName} first."
                ];
            }
        ...
        if (count($itemsWithRelations) > 0) {
            ...
            foreach ($itemsWithRelations as $item) {
                ...
                foreach ($relations as $relation) {
                    ...
                    $childIds = implode(', ', $relation['ids']);
                    $relationErrorMessages[] = [
                        'message' => "Item with ID {$itemId} has {$childCount} {$relationName} (rates : {$childIds}) and can't be deleted. Remove its {$relationName} first."
                    ];
                }
