...
class HMSAppointmentController extends Controller
{
    protected $interval;
    ...
    {
        $models = $this->model->whereType('doctor_appointment')->filter($request)->orderBy($request->order ? $request->order : 'updated_at', $request->sort ? $request->sort : 'DESC');
    ...
    public function available_times(Request $request)
    {
        ...
        $doctor_work_time = HMSDoctorsWorkTime::whereDayId($day_id)->whereDoctorId($request->doctor_id)->whereRoomId($request->room_id)->first();
        $time = [];
    ...
    public function get_next_patient_for_each_doctor(){
        $next_appointments_for_each_doctor = HMSAppointment::whereDate('date', now())->whereType('doctor_appointment')->whereStatus('Waiting')->orderBy('number')->get()->groupBy(['doctor_id','room_id'])
        ...
        $doctor_can_show_next_button =HMSAppointment::whereDate('date', now())->whereType('doctor_appointment')->whereIn('status',['Processing','Waiting'])->count();
    ...
    public function next_patient(Request $request){
        ...
        if($user && $user->type == 'doctor'){
            $processing_appointments = HMSAppointment::whereDate('date',now())->whereType('doctor_appointment')->whereStatus('Processing')->get();
            foreach($processing_appointments as $appoint){
                $appoint->update(['status' => 'Attended']);
                $this->notify_patient($appoint);
            }
            $current_patient = HMSAppointment::whereDate('date', now())->whereType('doctor_appointment')->whereStatus('Waiting')->orderBy('number')->first();
            if($current_patient){
    ...
    protected function notify_patient($appointment,$action = ''){
        ...
        if($appointment->email_notify){
            $lang = request()->header('Lang');
            $patient_name = $lang == 'ar' ? $patient->name : $patient->name_e;
            $doctor_name = $lang == 'ar' ? $doctor->name : $doctor->name_e;

            if($patient->email){
                $message = $this->get_message($appointment, $lang , $action,$doctor_name);
                Mail::to($patient->email)->send(new HMSNotifyPatientForAppointmentMail($message,$patient_name));
            }
            if($doctor->email){
                $message = $this->get_message_for_notify_doctor($appointment, $lang , $action,$patient_name);

                Mail::to($doctor->email)->send(new HMSNotifyPatientForAppointmentMail($message,$doctor_name));
            }
    ...
    protected function get_message($appointment, $lang, $action,$doctor_name){
    ...
    protected function get_message_for_notify_doctor($appointment, $lang, $action,$patient_name){
        $action = $action  == '' ? $appointment->status : $action;
        $appointment_time = $appointment->date . '  ( ' . $appointment->start_time->start . ' - ' . $appointment->end_time->end .' )';
        ...
        switch($action){
            ...
            case "Cancelled" : $message = $lang == "ar" ?
                "تم الغاء الموعد الخاص بك مع المريض $patient_name" : "Your appointment with $patient_name has been cancelled" ;
                break;
            case 'Added' :
                $message = $lang == 'ar' ?
                "تم حجز موعد مع المريض $patient_name في تاريخ $appointment_time " : "The appointment was booked with $patient_name on a date $appointment_time" ;
                break;
            case 'Extended' :
                $message = $lang == 'ar' ?
                "لقد تم تمديد موعدك مع المريض $patient_name  : $appointment_time " : "The time of your appointment with $patient_name has been extended $appointment_time" ;
                break;
            case 'Deleted' : $message = $lang == 'ar' ?
                "تم حذف الموعد الخاص بك مع المريض $patient_name" : "Your appointment with $patient_name has been deleted" ;
                break;
