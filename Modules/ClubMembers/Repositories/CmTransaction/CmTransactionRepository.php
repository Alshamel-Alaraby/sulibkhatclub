...
class CmTransactionRepository implements CmTransactionInterface
{
    ...
    public function reportCmTransactions($request)
    {
        ...
        $models = $this->model->join('cm_members', 'cm_members.id', 'cm_transactions.cm_member_id')->select('cm_transactions.*', 'cm_members.full_name', 'cm_members.id')->filter($request)->orderBy($request->order ? $request->order : 'updated_at', $request->sort ? $request->sort : 'DESC');
