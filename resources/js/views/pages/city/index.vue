...
export default {
  ...
  methods: {
    ...
    ExportExcel(type, fn, dl) {
      ...
      setTimeout(() => {
        ...
        if (dl) {
          ...
          XLSX.writeFile(
            wb,
            fn || ("City" + "." || "SheetJSTableExport.") + (type || "xlsx")
          );
        }
        this.enabled3 = true;
      }, 100);
    ...
    async resetModalEdit(id) {
      ...
      if (this.isVisible("country_id")) await this.getCategory();
      if (this.isVisible("governorate_id")) await this.getGovernorate(city.country.id);
      this.edit.name = city.name;
      this.edit.name_e = city.name_e;
      this.edit.is_active = city.is_active ? 1 : 0;
      if (city.country) this.edit.country_id = city.country.id;
      if (city.governorate) this.edit.governorate_id = city.governorate.id;
      this.errors = {};
    },
    /**
     *  hidden Modal (edit)
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-body">
                ...
                <div class="d-inline-block" style="width: 22.2%">
                  ...
                  <b-dropdown variant="primary" :text="$t('general.searchSetting')" ref="dropdown"
                    ...
                    <b-form-checkbox v-if="isVisible('name')" v-model="filterSetting" value="name" class="mb-1">{{
                      getCompanyKey("city_name_ar") }}</b-form-checkbox>
                    <b-form-checkbox v-model="filterSetting" value="name_e" v-if="isVisible('name_e')" class="mb-1">{{
                      getCompanyKey("city_name_en") }}</b-form-checkbox>
                    <b-form-checkbox v-model="filterSetting" v-if="isVisible('country_id')"
                      :value="$i18n.locale == 'ar' ? 'country.name' : 'country.name_e'" class="mb-1">{{
                        getCompanyKey("country") }}</b-form-checkbox>
                    <b-form-checkbox v-if="isVisible('governorate_id')" v-model="filterSetting" :value="
                      $i18n.locale == 'ar' ? 'governorate.name' : 'governorate.name_e'
                    " class="mb-1">{{ getCompanyKey("governorate") }}</b-form-checkbox>
                  </b-dropdown>
                  <!-- Basic dropdown -->
                </div>
            ...
            <div class="row justify-content-between align-items-center mb-2 px-1">
              <div class="col-md-3 d-flex align-items-center mb-1 mb-xl-0">
                ...
                <div class="d-inline-flex">
                  ...
                  <button class="custom-btn-dowonload" @click="$bvModal.show(`edit-${checkAll[0]}`)"
                    v-if="checkAll.length == 1">
                    <i class="mdi mdi-square-edit-outline"></i>
                  </button>
                  <!-- start mult delete  -->
                  <button class="custom-btn-dowonload" v-if="checkAll.length > 1" @click.prevent="deleteCity(checkAll)">
            ...
            <!--  create   -->
            <b-modal id="create" :title="getCompanyKey('city_create_form')" title-class="font-18" size="lg"
                ...
                <div class="row">
                  <div class="col-md-6" v-if="isVisible('country_id')">
                    <div class="form-group position-relative">
                      ...
                      <multiselect @input="getGovernorate(create.country_id)" v-model="create.country_id"
                  ...
                  <div class="col-md-6" v-if="isVisible('governorate_id')">
                    <div class="form-group position-relative">
                      ...
                      <multiselect @input="showGovernateModal" v-model="create.governorate_id"
                        :options="governorates.map((type) => type.id)" :custom-label="
                          (opt) => governorates.find((x) => x.id == opt).name
                        ">
                      </multiselect>
                      <div v-if="$v.create.governorate_id.$error || errors.governorate_id" class="text-danger">
                        {{ $t("general.fieldIsRequired") }}
                      </div>
                      <template v-if="errors.governorate_id">
                        <ErrorMessage v-for="(errorMessage, index) in errors.governorate_id" :key="index">{{ errorMessage
                        }}</ErrorMessage>
                      </template>
                    </div>
                  </div>
                  ...
                  <div class="col-md-6 direction-ltr" dir="ltr" v-if="isVisible('name_e')">
                    <div class="form-group">
                      ...
                      <input @keyup="englishValue(create.name_e)" type="text" class="form-control" data-create="2"
              ...
              <table class="table table-borderless table-hover table-centered m-0">
                ...
                <tbody v-if="cities.length > 0">
                  <tr @click.capture="checkRow(data.id)" @dblclick.prevent="$bvModal.show(`modal-edit-${data.id}`)"
                    v-for="(data, index) in cities" :key="data.id" :class="['body-tr-custom ', data.id]">
                    ...
                    <td v-if="enabled3" class="do-not-print">
                      <div class="btn-group">
                        ...
                        <div class="dropdown-menu dropdown-menu-custom">
                          <a class="dropdown-item" href="#" @click.prevent="$bvModal.show(`modal-edit-${data.id}`)">
                      ...
                      <!--  edit   -->
                      <b-modal :id="`modal-edit-${data.id}`" :title="getCompanyKey('city_edit_form')"
                        title-class="font-18" body-class="p-4" size="lg" :hide-footer="true"
                          ...
                          <div class="row">
                            <div class="col-md-6" v-if="isVisible('country_id')">
                              <div class="form-group position-relative">
                                ...
                                <multiselect @input="getGovernorate(edit.country_id)" v-model="edit.country_id"
                            ...
                            <div class="col-md-6" v-if="isVisible('governorate_id')">
                              <div class="form-group position-relative">
                                ...
                                <multiselect @input="showEditGovernateModal" v-model="edit.governorate_id"
                                  :options="governorates.map((type) => type.id)" :custom-label="
                                    (opt) => governorates.find((x) => x.id == opt).name
                                  ">
                                </multiselect>
                                ...
                                <template v-if="errors.governorate_id">
                                  <ErrorMessage v-for="(errorMessage, index) in errors.governorate_id" :key="index">{{
                                    errorMessage }}</ErrorMessage>
                                </template>
                              </div>
                            </div>
