...
export default {
    ...
    beforeRouteEnter(to, from, next) {
        next((vm) => {
            return permissionGuard(vm, "Daily Occupied Report", "all payer member club");
        });
    },
    methods: {
        formatDate(value) {
        ...
        getData(page = 1) {
            ...
            adminApi
                .get(
                    `/booking/report/occupied-rooms-status?page=${page}&per_page=${this.per_page}&branch_id=${this.member_type_id}`
                )
                .then((res) => {
                    let l = res.data;
                    this.items = l.data;
        ...
        getVoucherHeadersTotal(items){
            ...
            items.forEach(el => {
                if(el.document_header_details && el.document_header_details.length > 0){
                    sumItem += this.getTotalCharge(el);
                    total += parseFloat(this.getOutstandingBalance(el));
                    if(el.customer){
                        service += this.getVoucherHeaders(el.customer.voucher_headers);
                    }
                }
            });
        ...
        getVoucherHeaders(items){
            ...
            if(items.length > 0){
                sumItem = items.reduce((n, {amount}) => parseFloat(n) + parseFloat(amount), 0);
                return sumItem;
            }
            return sumItem;
        },
        getDocumentHeaders(items){
            let sumItem = 0;

            if(items.length > 0){
                sumItem = items.reduce((n, {net_invoice}) => n + net_invoice, 0);
        ...
        getTotalCharge(data){
            if(data.document_header_details.length > 0 && data.document_header_details){
                const date1 = new Date(data.document_header_details[0].date_from);
                const date2 = new Date();
                const diffTime = Math.abs(date2 - date1);
                const diffDaysPlus = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
                const diffDays = diffDaysPlus -1;

                return(
                    parseFloat((diffDays *(data.document_header_details[0].price_per_uint - (data.document_header_details[0].discount/data.document_header_details[0].rent_days))))
                    + this.getDocumentHeaders(data.customer.document_header)
                )
            }
            return 0;
        },
                    ...
                    <div class="card-body">
                        ...
                        <div ref="exportable_table" id="printData">
                            <div class="head-branch text-center">
                                ...
                                <p class="font-weight-bold">- DAILY OCCUPIED ROOMS REPORT -</p>
                                <div>
                                    {{ $t('general.branch') }}:
                                ...
                                >
                                    <thead>
                                        <tr>
                                            ...
                                            <th>
                                                <div class="d-flex justify-content-center">
                                                    <span>{{getCompanyKey("dailyCheckedIn_invoice_number")}}</span>
                                                </div>
                                            </th>
                                            <th>
                                                <div class="d-flex justify-content-center">
                                            ...
                                            <th>
                                                <div class="d-flex justify-content-center">
                                                    <span>{{getCompanyKey("dailyCheckedIn_accommodation_rate")}}</span>
                                                </div>
                                            </th>
                                            <th>
                                                <div class="d-flex justify-content-center">
                                                    <span>{{getCompanyKey("dailyCheckedIn_extra_guest")}}</span>
                                                </div>
                                            </th>
                                            <th>
                                                <div class="d-flex justify-content-center">
                                                    <span>{{getCompanyKey("dailyCheckedIn_service_charge")}}</span>
                                                </div>
                                            </th>
                                            <th>
                                                <div class="d-flex justify-content-center">
                                                    <span>{{getCompanyKey("dailyCheckedIn_discount")}}</span>
                                                </div>
                                            </th>
                                            <th>
                                                <div class="d-flex justify-content-center">
                                            ...
                                            <th>
                                                <div class="d-flex justify-content-center">
                                                    <span>{{getCompanyKey("dailyCheckedIn_total_charges")}}</span>
                                                </div>
                                            </th>
                                            <th>
                                                <div class="d-flex justify-content-center">
                                                    <span>{{getCompanyKey("dailyCheckedIn_total_payment")}}</span>
                                                </div>
                                            </th>
                                            <th>
                                                <div class="d-flex justify-content-center">
                                                    <span>{{getCompanyKey("dailyCheckedIn_outstanding_balance")}}</span>
                                                </div>
                                            </th>
                                        </tr>
                                    </thead>
                                    ...
                                    >
                                        ...
                                        <td>
                                            {{
                                                data.customer ?
                                                    getDocumentHeaders(data.customer.document_header)
                                                    : '0'
                                            }}
                                        </td>
                                        <td>
                                            {{
                                                data.document_header_details?
                                                    data.document_header_details[0] ?
                                                        (data.document_header_details[0].discount/data.document_header_details[0].rent_days)
                                                        : '-'
                                                    : '-'
                                            }}
                                        </td>
                                        <td>
                                            {{
                                                data.document_header_details?
                                                    data.document_header_details[0] ?
                                                        (data.document_header_details[0].price_per_uint - (data.document_header_details[0].discount/data.document_header_details[0].rent_days))
                                                        : '-'
                                                    : '-'
                                            }}
                                        </td>
                                        ...
                                        <td>{{ getOutstandingBalance(data) ?? 0 }}</td>
                                    </tr>
                                    <tr>
                                        <td colspan="12"></td>
