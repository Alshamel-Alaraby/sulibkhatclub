...
export default {
    ...
    methods: {
        ...
        getData(page = 1) {
            ...
            adminApi
                .get(
                    `/categories?page=${page}&per_page=${this.per_page}&search=${this.search}&${filter}`
                )
                .then((res) => {
                    let l = res.data;
            ...
            if (Array.isArray(id)) {
                Swal.fire({
                    ...
                    if (result.value) {
                        this.isLoader = true;
                        adminApi
                            .post(`/categories/bulk-delete`, { ids: id })
                            .then((res) => {
                                this.checkAll = [];
                                this.getData();
                                Swal.fire({
        ...
        ExportExcel(type, fn, dl) {
            ...
            setTimeout(() => {
                ...
                if (dl) {
                    ...
                    XLSX.writeFile(wb, fn || (('category' + '.' || 'SheetJSTableExport.') + (type || 'xlsx')));
                }
                this.enabled3 = true;
            }, 100);
        },
