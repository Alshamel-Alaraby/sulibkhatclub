...
export default {
  page: {
    title: "Look ups",
    meta: [{ name: "description", content: "Look ups" }],
  },
  mixins: [translation],
  components: {
    Layout,
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      if (
        vm.$store.state.auth.work_flow_trees.includes("board rent") ||
  ...
  methods: {
    ...
    log(id) {
      if (this.mouseEnter != id) {
        ...
        this.mouseEnter = id;
        adminApi
          .get(`/boards-rent/look_ups/logs/${id}`)
          .then((res) => {
            let l = res.data.data;
            l.forEach((e) => {
    ...
    getData(page = 1) {
      ...
      adminApi
        .get(
          `/boards-rent/look_ups?page=${page}&per_page=${this.per_page}&search=${this.search}&${filter}`
        )
        .then((res) => {
          let l = res.data;
          this.lookups = l.data;
          this.lookupsPagination = l.pagination;
    ...
    getDataCurrentPage() {
      if (
        this.current_page <= this.lookupsPagination.last_page &&
        this.current_page != this.lookupsPagination.current_page &&
        this.current_page
      ) {
        this.isLoader = true;
        let filter = "";
        ...
        adminApi
          .get(
            `/boards-rent/look_ups?page=${this.current_page}&per_page=${this.per_page}&search=${this.search}&${filter}`
          )
          .then((res) => {
            let l = res.data;
            this.lookups = l.data;
      ...
      if (Array.isArray(id)) {
        Swal.fire({
          ...
          if (result.value) {
            this.isLoader = true;
            adminApi
              .post(`/boards-rent/look_ups/bulk-delete`, { ids: id })
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            adminApi
              .delete(`/boards-rent/look_ups/${id}`)
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
    ...
    async resetModalEdit(id) {
      this.getRootNodes();
      let propertyTree = this.lookups.find((e) => id == e.id);
      this.edit.name = propertyTree.name;
      this.edit.name_e = propertyTree.name_e;
      this.edit.type = propertyTree.type;
        ...
        <div class="card">
          <div class="card-body">
            <div class="row justify-content-between align-items-center mb-2">
              <h4 class="header-title">{{ $t("general.lookupsTable") }}</h4>
                ...
                <div class="d-inline-block" style="width: 22.2%">
                  ...
                  >
                    ...
                    <b-form-checkbox
                      ...
                      value="name_e"
                      class="mb-1"
                      >{{ getCompanyKey("boardRent_lookUp_name_en") }}</b-form-checkbox>
                      <b-form-checkbox
                          v-model="filterSetting"
                          value="type"
                          class="mb-1"
                      >{{ getCompanyKey("boardRent_lookUp_type") }}</b-form-checkbox>
                  </b-dropdown>
                  <!-- Basic dropdown -->
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <b-form-checkbox v-model="setting.parent" class="mb-1">
                      {{ getCompanyKey("boardRent_lookUp_parent") }}
                    </b-form-checkbox>
                  ...
                  <div class="d-inline-flex align-items-center pagination-custom">
                    <div class="d-inline-block" style="font-size: 15px">
                      {{ lookupsPagination.from }}-{{
                        lookupsPagination.to
                      }}
                      /
                      {{ lookupsPagination.total }}
                    </div>
                    <div class="d-inline-block">
                      <a
                        ...
                        :style="{
                          'pointer-events':
                            lookupsPagination.current_page == 1 ? 'none' : '',
                        }"
                        @click.prevent="
                          getData(lookupsPagination.current_page - 1)
                        "
                      >
                        <span>&lt;</span>
                      </a>
                      ...
                      <a
                        ...
                        :style="{
                          'pointer-events':
                            lookupsPagination.last_page ==
                            lookupsPagination.current_page
                              ? 'none'
                              : '',
                        }"
                        @click.prevent="
            ...
            <b-modal
              id="create"
              :title="getCompanyKey('boardRent_lookUp_create_form')"
              title-class="font-18"
              body-class="p-4 "
              :hide-footer="true"
              size="lg"
            ...
            <div class="table-responsive mb-3 custom-table-theme position-relative">
              ...
              <table class="table table-borderless table-hover table-centered m-0" ref="exportable_table"
                  ...
                  <tr>
                    ...
                    <th v-if="setting.name">
                      <div class="d-flex justify-content-center">
                        <span>{{ getCompanyKey("boardRent_lookUp_name_ar") }}</span>
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="lookups.sort(sortString('name'))"
                          ></i>
                          <i
                    ...
                    <th v-if="setting.name_e">
                      <div class="d-flex justify-content-center">
                        <span>{{ getCompanyKey("boardRent_lookUp_name_en") }}</span>
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="lookups.sort(sortString('name_e'))"
                          ></i>
                          <i
                            class="fas fa-arrow-down"
                            @click="lookups.sort(sortString('-name_e'))"
                    ...
                    <th v-if="setting.type">
                      <div class="d-flex justify-content-center">
                        <span>{{ getCompanyKey("boardRent_lookUp_type") }}</span>
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="lookups.sort(sortString('required'))"
                          ></i>
                          <i
                            class="fas fa-arrow-down"
                            @click="lookups.sort(sortString('-required'))"
                ...
                <tbody v-if="lookups.length > 0">
                  <tr
                    @click.capture="checkRow(data.id)"
                    @dblclick.prevent="$bvModal.show(`modal-edit-${data.id}`)"
                    v-for="(data, index) in lookups"
                    :key="data.id"
                  ...
                  >
                    ...
                    <td v-if="enabled3" class="do-not-print">
                      ...
                      <b-modal
                        ...
                        :title="getCompanyKey('boardRent_lookUp_edit_form')"
                        title-class="font-18"
                        body-class="p-4"
                        size="lg"
                        :ref="`edit-${data.id}`"
