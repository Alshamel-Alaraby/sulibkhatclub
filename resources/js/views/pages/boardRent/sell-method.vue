...
export default {
  page: {
    title: "Sell Methods",
    meta: [{ name: "description", content: "Sell Methods" }],
  },
  mixins: [translation],
  components: {
    Layout,
 ...
 beforeRouteEnter(to, from, next) {
    next((vm) => {
      if (
        vm.$store.state.auth.work_flow_trees.includes("sell method") ||
   ...
   methods: {
    ...
    log(id) {
      if (this.mouseEnter != id) {
        ...
        this.mouseEnter = id;
        adminApi
          .get(`/boards-rent/sell-methods/logs/${id}`)
          .then((res) => {
            let l = res.data.data;
            l.forEach((e) => {
    ...
    getData(page = 1) {
      ...
      adminApi
        .get(
          `/boards-rent/sell-methods?page=${page}&per_page=${this.per_page}&search=${this.search}&${filter}`
        )
        .then((res) => {
          let l = res.data;
          this.sellMethods = l.data;
          this.sellMethodsPagination = l.pagination;
    ...
    getDataCurrentPage(page = 1) {
      if (
        this.current_page <= this.sellMethodsPagination.last_page &&
        this.current_page != this.sellMethodsPagination.current_page &&
        this.current_page
      ) {
        this.isLoader = true;
        let filter = "";
        ...
        adminApi
          .get(
            `/boards-rent/sell-methods?page=${this.current_page}&per_page=${this.per_page}&search=${this.search}&${filter}&company_id=${this.company_id}`
          )
          .then((res) => {
            let l = res.data;
            this.sellMethods = l.data;
      ...
      if (Array.isArray(id)) {
        Swal.fire({
          ...
          if (result.value) {
            this.isLoader = true;
            adminApi
              .post(`/boards-rent/sell-methods/bulk-delete`, { ids: id })
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            adminApi
              .delete(`/boards-rent/sell-methods/${id}`)
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
    ...
    editSubmit(id) {
      ...
      if (this.$v.edit.$invalid) {
        ...
        this.errors = {};
        adminApi
          .put(`/boards-rent/sell-methods/${id}`, {
            ...this.edit,
            target_calculation_ratio: this.edit.calculated_percentage,
            commission_ratio: this.edit.commission_ratio,
          })
          .then((res) => {
     ...
     */
    resetModalEdit(id) {
      let sellMethods = this.sellMethods.find((e) => id == e.id);
      this.edit.name = sellMethods.name;
      this.edit.name_e = sellMethods.name_e;
      this.edit.calculated_percentage = sellMethods.target_calculation_ratio;
      this.edit.commission_ratio = sellMethods.commission_ratio;
      this.errors = {};
    },
    /**
     *  hidden Modal (edit)
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row justify-content-between align-items-center mb-2">
              <h4 class="header-title">{{ $t("general.sellMethodsTable") }}</h4>
                ...
                <div class="d-fex">
                  ...
                  <div class="d-inline-block">
                    ...
                    >
                      ...
                      <b-form-checkbox
                        v-model="setting.calculated_percentage"
                        class="mb-1"
                      >
                        {{
                          getCompanyKey(
                      ...
                      <b-form-checkbox
                        v-model="setting.commission_ratio"
                        class="mb-1"
                      >
                        {{
                          getCompanyKey(
                  ...
                  >
                    <div class="d-inline-block" style="font-size: 13px">
                      {{ sellMethodsPagination.from }}-{{
                        sellMethodsPagination.to
                      }}
                      /
                      {{ sellMethodsPagination.total }}
                    </div>
                    <div class="d-inline-block">
                      <a
                        href="javascript:void(0)"
                        :style="{
                          'pointer-events':
                            sellMethodsPagination.current_page == 1
                              ? 'none'
                              : '',
                        }"
                        @click.prevent="
                          getData(sellMethodsPagination.current_page - 1)
                        "
                      >
                        <span>&lt;</span>
                      </a>
                      ...
                      <a
                        ...
                        :style="{
                          'pointer-events':
                            sellMethodsPagination.last_page ==
                            sellMethodsPagination.current_page
                              ? 'none'
                              : '',
                        }"
                        @click.prevent="
                  ...
                  <div class="col-md-12">
                    <div class="form-group">
                      ...
                      <input
                        ...
                        :class="{
                          'is-invalid':
                            $v.create.commission_ratio.$error ||
              ...
              >
                <thead>
                  <tr>
                      ...
                      <div class="d-flex justify-content-center">
                        ...
                        <div class="arrow-sort">
                          <i
                            ...
                            @click="sellMethods.sort(sortString('name'))"
                          ></i>
                          <i
                    ...
                    <th v-if="setting.name_e">
                      <div class="d-flex justify-content-center">
                        ...
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="sellMethods.sort(sortString('name_e'))"
                          ></i>
                          <i
                            class="fas fa-arrow-down"
                            @click="sellMethods.sort(sortString('-name_e'))"
                    ...
                    <th v-if="setting.calculated_percentage">
                      <div class="d-flex justify-content-center">
                        <span>{{
                          getCompanyKey(
                            "boardRent_sellMethod_calculated_percentage"
                    ...
                    <th v-if="setting.commission_ratio">
                      <div class="d-flex justify-content-center">
                        <span>{{
                          getCompanyKey("boardRent_sellMethod_commission_ratio")
                        }}</span>
                ...
                <tbody v-if="sellMethods.length > 0">
                  <tr
                    @click.capture="checkRow(data.id)"
                    @dblclick.prevent="$bvModal.show(`modal-edit-${data.id}`)"
                    v-for="(data, index) in sellMethods"
                    :key="data.id"
                  ...
                  >
                    ...
                    <td v-if="setting.calculated_percentage">
                      <h5 class="m-0 font-weight-normal">
                        {{ data.target_calculation_ratio }}
                      </h5>
                    </td>
                    <td v-if="setting.commission_ratio">
                      <h5 class="m-0 font-weight-normal">
                            ...
                            <div class="col-md-12">
                              <div class="form-group">
                                ...
                                <input
                                  ...
                                  class="form-control"
                                  data-create="2"
                                  v-model="$v.edit.calculated_percentage.$model"
                                  :class="{
                                    'is-invalid':
                                      $v.edit.calculated_percentage.$error ||
                                      errors.calculated_percentage,
                                    'is-valid':
                                      !$v.edit.calculated_percentage.$invalid &&
                                      !errors.calculated_percentage,
                                  }"
                                  id="field-2"
                                />
                            ...
                            <div class="col-md-12">
                              <div class="form-group">
                                ...
                                <input
                                  ...
                                  class="form-control"
                                  data-create="2"
                                  v-model="$v.edit.commission_ratio.$model"
                                  :class="{
                                    'is-invalid':
                                      $v.edit.commission_ratio.$error ||
                                      errors.commission_ratio,
                                    'is-valid':
                                      !$v.edit.commission_ratio.$invalid &&
                                      !errors.commission_ratio,
                                  }"
                                  id="field-2"
                                />
