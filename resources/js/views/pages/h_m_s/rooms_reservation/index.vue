...
export default {
    ...
    components: {
        Layout, PageHeader, loader, searchPage, appointmentTable, patient_details, InsuranceCompany,ModalDoctor,ModalPatient,ModalBranch,
        actionSetting, tableCustom, Modal
    },
    beforeRouteEnter(to, from, next) {
        next((vm) => {
            return permissionGuard(vm, "Rooms Reservation", "all Rooms Reservation");
        });
    },
    data() {
        return {
            url: '/h_m_s/rooms_reservation',
            debounce: {},
            objPagination: {},
            page_title: {},
            per_page: 50,
            ...
            drugs: [],
            diagnosis_tests: [],
            opreations_rooms_categories_data: [],
    ...
    mounted() {
        ...
        this.get_patients();
        this.getBranch();
        this.opreations_rooms_categories();
    ...
    methods: {
        ...
        async get_appointments(page = 1) {
            ...
            &columns[10]=date&columns[11]=created_at`).then((res) => {
        ...
        opreations_rooms_categories() {
            adminApi.get(`h_m_s/opreations_rooms_categories?drop_down=1&company_id=${this.company_id}`).then((res) => {
                this.opreations_rooms_categories_data = res.data.data
            });
        },
                    ...
                    <div class="card-body">
                        ...
                        <div class="row justify-content-between align-items-center mb-2">
                            <h4 class="header-title">{{ page_title && page_title.url == $route.fullPath ? ($i18n.locale ==
                                'ar' ? page_title.title : page_title.title_e) : $t('general.Rooms Reservation') }}</h4>
                        ...
                        <div class="col-lg-4 d-flex my-2" style="font-weight: 500">
                            <div class="d-inline-block">
                                <b-button @click.prevent="$bvModal.show(`create`);" variant="primary"
                                    v-if="isPermission('create Rooms Reservation')" class="font-weight-bold">
                                    {{ $t("general.Create") }}
                        ...
                        <appointmentTable :appointments="appointments" :doctors="doctors" :patients="patients"
                            ...
                            :drugs="drugs" :permissionUpdate="isPermission('update Rooms Reservation')"
                            :permissionDelete="isPermission('delete Rooms Reservation')"
                            :permissionCreatePrescription="false"
                        ...
                        <Modal :id="'create'" :companyKeys="companyKeys" :defaultsKeys="defaultsKeys" :url="url"
                            :doctors="doctors" :patients="patients" :branches="branches"
                            :opreations_rooms_categories="opreations_rooms_categories_data" :isPage="true"
                            :isVisiblePage="isVisible" :isRequiredPage="isRequired" :type="type"
                            @getDataTable="get_appointments(1)" :isPermission="isPermission"
                            v-if="isPermission('create Rooms Reservation')" />
                        ...
                        <ModalBranch :tables="[]" :companyKeys="companyKeys" :defaultsKeys="defaultsKeys"
                            id="addAppointmentBranch" :isPage="false" type="create" :isPermission="isPermission"
                            @created="getBranch" />


                        <ModalDoctor :companyKeys="companyKeys" :defaultsKeys="defaultsKeys" id="addDoctorFromAppointment"
                            :isPage="false" type="create" :isPermission="isPermission" @created="get_doctors" />
