<template>
   <div>
       ...
       <b-modal
           :id="id"
           :title="type != 'edit'?getCompanyKey('hms_diagnosis_tests_create_form'):getCompanyKey('hms_diagnosis_tests_edit_form')"
           title-class="font-18"
           body-class="p-4 "
           :hide-footer="true"
           @show="resetModal"
       ...
       >
           <form>
               ...
               <div class="row">
                   <div class="col-md-12" v-if="isVisible('name')">
                       <div class="form-group">
                           <label for="field-1" class="control-label">
                               {{ getCompanyKey('hms_diagnosis_tests_name_ar') }}
                   ...
                   <div class="col-md-12" v-if="isVisible('name_e')">
                       <div class="form-group">
                           <label for="field-2" class="control-label">
                               {{ getCompanyKey('hms_diagnosis_tests_name_en') }}
                   ...
                   <div class="col-md-12" v-if="isVisible('price')">
                        <div class="form-group">
                   ...
                   <div class="col-md-12" v-if="isVisible('comment')">
                       <div class="form-group">
                           <label for="field-3" class="control-label">
                               {{ getCompanyKey('hms_diagnosis_tests_comment') }}
                               <span v-if="isRequired('comment')" class="text-danger">*</span>
                           </label>
                           <div >
                                <textarea class="form-control "
                                   data-create="3"
                                   v-model="$v.create.comment.$model"
                                   :class="{
                                        'is-invalid': $v.create.comment.$error || errors.comment,
                                        'is-valid': !$v.create.comment.$invalid && !errors.comment,
                                    }"
                                   id="field-3"
                                    cols="30" rows="5">
                           ...
                           <template v-if="errors.comment">
                               <ErrorMessage
                                   v-for="(errorMessage, index) in errors.comment"
                                   :key="index"
                               >{{ errorMessage }}</ErrorMessage
                               >
                           </template>
...
export default {
    ...
    props: {
        ...
        type: {default: 'create'}, idObjEdit: {default: null},isPermission: {},url: {default: '/h_m_s/diagnosis_tests'}
    },
    data() {
        return {
            fields: [],
    ...
    methods: {
        getCustomTableFields() {
            this.isCustom = true;
            adminApi
                .get(`/customTable/table-columns/h_m_s_diagnosis_tests`)
                .then((res) => {
            ...
            setTimeout( () => {
                if(this.type != 'edit'){
                    ...
                    if(this.idObjEdit.dataObj){
                        let diagnosis_test = this.idObjEdit.dataObj;
                        this.errors = {};
                        this.create.name = diagnosis_test.name;
                        this.create.name_e = diagnosis_test.name_e;
                        this.create.price = diagnosis_test.price;
                        this.create.comment = diagnosis_test.comment;
                    }
                }
            },50);
        },
