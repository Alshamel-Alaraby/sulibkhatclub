...
export default {
  page: {
    ...
    meta: [{ name: "description", content: "Pending member" }],
  },
  mixins: [translation],
  components: {
    Layout,
  ...
  watch: {
    ...
    isCheckAll(after, befour) {
      if (after) {
        this.pendingMembers.forEach((el) => {
  ...
  methods: {
    ...
    log(id) {
      if (this.mouseEnter != id) {
        ...
        this.mouseEnter = id;
        adminApi
          .get(`/club-members/pending-members/logs/${id}`)
          .then((res) => {
            let l = res.data.data;
            l.forEach((e) => {
    ...
    getData(page = 1) {
      ...
      adminApi
        .get(
          `/club-members/pending-members?page=${page}&per_page=${this.per_page}&search=${this.search}&${filter}`
        )
        .then((res) => {
          let l = res.data;
          this.pendingMembers = l.data;
          this.pendingMembersPagination = l.pagination;
    ...
    getCustomTableFields() {
      adminApi
        .get(`/customTable/table-columns/general_pendingMembers`)
        .then((res) => {
    ...
    getDataCurrentPage(page = 1) {
      if (
        this.current_page <= this.pendingMembersPagination.last_page &&
        this.current_page != this.pendingMembersPagination.current_page &&
        this.current_page
      ) {
        this.isLoader = true;
        let filter = "";
        ...
        adminApi
          .get(
            `/club-members/pending-members?page=${this.current_page}&per_page=${this.per_page}&search=${this.search}&${filter}`
          )
          .then((res) => {
            let l = res.data;
            this.pendingMembers = l.data;
      ...
      if (Array.isArray(id)) {
        Swal.fire({
          ...
          if (result.value) {
            this.isLoader = true;
            adminApi
              .post(`/club-members/pending-members/bulk-delete`, { ids: id })
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            adminApi
              .delete(`/club-members/pending-members/${id}`)
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
    ...
    editSubmit(id) {
      ...
      if (this.$v.edit.$invalid) {
        ...
        adminApi
          .put(`/club-members/pending-members/${id}`, data)
          .then((res) => {
     ...
     */
    resetModalEdit(id) {
      let branch = this.pendingMembers.find((e) => id == e.id);
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-body">
                  ...
                  <div class="d-inline-flex align-items-center pagination-custom">
                    ...
                    <div class="d-inline-block">
                      ...
                      <a
                        ...
                        :style="{
                          'pointer-events':
                            pendingMembersPagination.last_page ==
                            pendingMembersPagination.current_page
                              ? 'none'
                              : '',
                        }"
                        @click.prevent="getData(pendingMembersPagination.current_page + 1)"
            ...
            <b-modal
              id="create"
              :title="getCompanyKey('pending_member_create_form')"
              title-class="font-18"
              body-class="p-4 "
              :hide-footer="true"
              @show="resetModal"
                    ...
                    <div class="form-group">
                      ...
                      >
                        ...
                        <b-form-radio
                          ...
                          name="some-radios"
                          value="inactive"
                          >{{ $t("general.inpending") }}</b-form-radio
                        >
                      </b-form-group>
            ...
            <div class="table-responsive mb-3 custom-table-theme position-relative">
              ...
              >
                      ...
                      <div class="d-flex justify-content-center">
                        ...
                        <div class="arrow-sort">
                          <i
                            ...
                            @click="pendingMembers.sort(sortString('name'))"
                          ></i>
                          <i
                    ...
                    <th v-if="setting.name_e ">
                      <div class="d-flex justify-content-center">
                        ...
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="pendingMembers.sort(sortString('name_e'))"
                          ></i>
                          <i
                            class="fas fa-arrow-down"
                            @click="pendingMembers.sort(sortString('-name_e'))"
                ...
                <tbody v-if="pendingMembers.length > 0">
                  <tr
                    @click.capture="checkRow(data.id)"
                    @dblclick.prevent="$bvModal.show(`modal-edit-${data.id}`)"
                    v-for="(data, index) in pendingMembers"
                    :key="data.id"
                  ...
                  >
                    ...
                    <td v-if="enabled3" class="do-not-print">
                      ...
                      <b-modal
                        ...
                        :title="getCompanyKey('pending_member_edit_form')"
                        title-class="font-18"
                        body-class="p-4"
                        :ref="`edit-${data.id}`"
                        :hide-footer="true"
