...
export default {
    page: {
        ...
        meta: [{name: "description", content: "Print Committee Lists"}],
    },
    mixins: [translation],
    components: {
        Multiselect,
    ...
    beforeRouteEnter(to, from, next) {
        next((vm) => {
            return permissionGuard(vm, "Print Committee Lists", "all Permission member club");
        });
    ...
    watch: {
        ...
        '$v.create.cm_permission_id.$model': function (newValue, oldValue) {
    ...
    methods: {
        ...
        getData(page = 1) {
            ...
            if (this.$v.create.$invalid) {
                ...
                adminApi
                    .get(`/club-members/reports/members-Committee?members_permissions_id=${this.create.cm_permission_id}&dateOfYear=${startDate}&page=${page}`, {
                        params: {
        ...
        getDataCurrentPage(page = 1) {
            ...
            if (
                ...
                if (this.$v.create.$invalid) {
                    ...
                    adminApi
                        .get(`/club-members/reports/members-Committee?members_permissions_id=${this.create.cm_permission_id}&dateOfYear=${startDate}&page=${this.current_page}&search=${this.search}&${filter}`, {
                            params: {
                                members_permissions_id: this.create.cm_permission_id,
                                dateOfYear: this.create.date
                            },
        ...
        getCommitee() {
            this.books = this.items.find(el => el.id == this.item_id).members_data;
            this.isLoader = true;
            setTimeout(() => {
                this.isLoader = false
            }, 500)
                        ...
                        >
                            <form>
                                ...
                                <div class="row justify-content-center">
                                    ...
                                    <div v-if="showDiv" class="col-md-12">
                                        <div class="form-group">
                                            <label class="control-label">
                                                {{ $t('general.ElectoralCommittees') }}
                                                <span class="text-danger">*</span>
                                            </label>
                                            <multiselect
                                                @input="openButton"
                                                ...
                                                :custom-label="(opt) =>$i18n.locale == 'ar'? items.find((x) => x.id == opt).name: items.find((x) => x.id == opt).name_e">
                                            </multiselect>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="table-responsive mb-3 custom-table-theme position-relative" ref="exportable_table"
                                ...
                                <div>
                                    ...
                                    <div class="d-inline-block" style="width:70%">
                                        ...
                                        <div class="text-center" v-if="items.length > 0">
                                            <span v-if="items.find(el => el.id == item_id)">
                                                {{
                                                    $i18n.locale == 'ar' ? items.find(el => el.id == item_id).name : items.find(el => el.id == item_id).name_e
                                                }}
                                            </span>
                                        </div>
                                        <div class="text-center" v-if="items.length > 0">
                                            <span v-if="items.find(el => el.id == item_id)">
                                                {{
                                                    $t('general.first_member')
                                                }}: {{ items.find(el => el.id == item_id).first_member }}
                                            </span>
                                        </div>
                                        <div class="text-center" v-if="items.length > 0">
                                            <span v-if="items.find(el => el.id == item_id)">
                                                {{
                                                    $t('general.last_member')
                                                }}: {{ items.find(el => el.id == item_id).last_member }}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <table class="table table-borderless table-hover table-centered m-0"
                                ...
                                <tr>
                                    ...
                                    <th colspan="3">{{ $t('general.NewManagerSignature') }}</th>
                                </tr>
                                </thead>
                                ...
                                >
                                    ...
                                    <td v-if="setting.PaymentDate">
                                        {{
                                            data.last_cm_transaction.date ? formatDate(data.last_cm_transaction.date) : '---'
                                        }}
                                    </td>
                                    <td v-if="setting.document_no">
                                        {{
                                            data.last_cm_transaction.document_no ? data.last_cm_transaction.document_no : '---'
                                        }}
                                    </td>
                                    <td></td>
                                </tr>
