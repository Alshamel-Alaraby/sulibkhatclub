...
export default {
  ...
  methods: {
    ...
    getData(page = 1) {
      ...
      adminApi
        .get(
          `document-statuses?page=${page}&per_page=${this.per_page}&search=${this.search}&${filter}`
        )
        .then((res) => {
          let l = res.data;
    ...
    getDataCurrentPage(page = 1) {
      if (
        ...
        adminApi
          .get(
            `document-statuses?page=${this.current_page}&per_page=${this.per_page}&search=${this.search}&${filter}&company_id=${this.company_id}`
          )
          .then((res) => {
            let l = res.data;
            this.unitStatuses = l.data;
      ...
      if (Array.isArray(id)) {
        Swal.fire({
          ...
          if (result.value) {
            this.isLoader = true;
            adminApi
              .post(`document-statuses/bulk-delete`, { ids: id })
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row justify-content-between align-items-center mb-2">
              <h4 class="header-title">{{ $t("general.documentStatusTable") }}</h4>
                ...
                <div class="d-inline-block" style="width: 22.2%">
                  ...
                  <b-dropdown variant="primary" :text="$t('general.searchSetting')" ref="dropdown"
                    ...
                    <b-form-checkbox v-if="isVisible('name_e')" v-model="filterSetting" value="name_e" class="mb-1">{{
                      getCompanyKey("document_status_name_en") }}</b-form-checkbox>
                  </b-dropdown>
                  <!-- Basic dropdown -->
                </div>
                ...
                <div class="d-fex">
                  ...
                  <div class="d-inline-block">
                    ...
                    <b-dropdown variant="primary" :html="`${$t('general.setting')} <i class='fe-settings'></i>`"
                      ...
                      <b-form-checkbox v-if="isVisible('name')" v-model="setting.name" class="mb-1">{{ getCompanyKey("document_status_name_ar") }}
                      </b-form-checkbox>
            ...
            <!--  create   -->
            <b-modal id="create" :title="getCompanyKey('document_status_create_form')" title-class="font-18" body-class="p-4 "
                  ...
                  <tr>
                    ...
                    <th v-if="setting.name && isVisible('name')">
                      <div class="d-flex justify-content-center">
                        <span>{{ getCompanyKey("document_status_name_ar") }}</span>
                        <div class="arrow-sort">
                          <i class="fas fa-arrow-up" @click="unitStatuses.sort(sortString('name'))"></i>
                          <i class="fas fa-arrow-down" @click="unitStatuses.sort(sortString('-name'))"></i>
                        </div>
                      </div>
                    </th>
                    <th v-if="setting.name_e && isVisible('name_e')">
                      <div class="d-flex justify-content-center">
                        <span>{{ getCompanyKey("document_status_name_en") }}</span>
                        <div class="arrow-sort">
                          <i class="fas fa-arrow-up" @click="unitStatuses.sort(sortString('name_e'))"></i>
                          <i class="fas fa-arrow-down" @click="unitStatuses.sort(sortString('-name_e'))"></i>
                        </div>
                      </div>
                    </th>
                    <th v-if="setting.is_active && isVisible('is_active')">
                      <div class="d-flex justify-content-center">
                        <span>{{ getCompanyKey("document_status_status") }}</span>
                        <div class="arrow-sort">
                          <i class="fas fa-arrow-up" @click="unitStatuses.sort(sortString('is_active'))"></i>
                          <i class="fas fa-arrow-down" @click="unitStatuses.sort(sortString('-is_active'))"></i>
                        </div>
                      </div>
                    </th>
                    <th v-if="setting.is_default && isVisible('is_default')">
                      <div class="d-flex justify-content-center">
                        <span>{{ getCompanyKey("document_status_default") }}</span>
                        <div class="arrow-sort">
                          <i class="fas fa-arrow-up" @click="unitStatuses.sort(sortString('is_default'))"></i>
                          <i class="fas fa-arrow-down" @click="unitStatuses.sort(sortString('-is_default'))"></i>
                        </div>
                      </div>
                    </th>
                ...
                <tbody v-if="unitStatuses.length > 0">
                  <tr @click.capture="checkRow(data.id)" @dblclick.prevent="$bvModal.show(`modal-edit-${data.id}`)"
                    ...
                    <td v-if="enabled3" class="do-not-print">
                      ...
                      <!--  edit   -->
                      <b-modal :id="`modal-edit-${data.id}`" :title="getCompanyKey('document_status_edit_form')"
                              ...
                              <div class="form-group">
                                ...
                                <div dir="rtl">
                                  <input @keyup="arabicValue(edit.name)" type="text" class="form-control" data-edit="1"
                                    @keypress.enter="moveInput('input', 'edit', 2)" v-model="$v.edit.name.$model" :class="{
                              ...
                              <div class="form-group">
                                ...
                                <div dir="ltr">
                                  <input @keyup="englishValue(edit.name_e)" type="text" class="form-control englishInput"
                                    data-edit="2" @keypress.enter="moveInput('select', 'edit', 3)"
                              ...
                              <div class="form-group">
                                ...
                                <b-form-group :class="{
                                  ...
                                  <b-form-radio class="d-inline-block" v-model="$v.edit.is_default.$model"
                                    name="edit_def_some-radios" value="1">{{ $t("general.Default") }}</b-form-radio>
                                  <b-form-radio class="d-inline-block m-1" v-model="$v.edit.is_default.$model"
                                    name="edit_def_some-radios" value="0">{{ $t("general.Nondefault") }}</b-form-radio>
                                </b-form-group>
                                <template v-if="errors.is_default">
