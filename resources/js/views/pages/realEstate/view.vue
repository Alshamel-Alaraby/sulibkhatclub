...
export default {
  ...
  methods: {
    ...
    async getBranch() {
      ...
      await adminApi
        .get(`/real-estate/view?company_id=${this.company_id}`)
        .then((res) => {
          let l = res.data.data;
        })
        .catch((err) => {
    ...
    getData(page = 1) {
      ...
      adminApi
        .get(
          `/real-estate/view?page=${page}&per_page=${this.per_page}&search=${this.search}&${filter}`
        )
        .then((res) => {
          let l = res.data;
          this.views = l.data;
          this.viewsPagination = l.pagination;
    ...
    getDataCurrentPage(page = 1) {
      if (
        this.current_page <= this.viewsPagination.last_page &&
        this.current_page != this.viewsPagination.current_page &&
        this.current_page
      ) {
        this.isLoader = true;
        let filter = "";
      ...
      if (Array.isArray(id)) {
        Swal.fire({
          ...
          if (result.value) {
            this.isLoader = true;
            adminApi
              .post(`/real-estate/view/bulk-delete`, { ids: id })
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
    ...
    AddSubmit() {
      ...
      if (this.$v.create.$invalid) {
        ...
        this.errors = {};
        adminApi
          .post(`/real-estate/view`, this.create)
          .then((res) => {
    ...
    editSubmit(id) {
      ...
      if (this.$v.edit.$invalid) {
        ...
        this.errors = {};
        adminApi
          .put(`/real-estate/view/${id}`, this.edit)
          .then((res) => {
     ...
     */
    resetModalEdit(id) {
      let branch = this.views.find((e) => id == e.id);
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row justify-content-between align-items-center mb-2">
              <h4 class="header-title">{{ $t("general.viewsTable") }}</h4>
                ...
                <div class="d-inline-block" style="width: 22.2%">
                  ...
                  >
                    <b-form-checkbox
                      ...
                      value="name"
                      class="mb-1"
                      >{{ getCompanyKey("view_name_ar") }}</b-form-checkbox
                    >
                    <b-form-checkbox
                      ...
                      value="name_e"
                      class="mb-1"
                      >{{ getCompanyKey("view_name_en") }}</b-form-checkbox
                    >
                  </b-dropdown>
                  <!-- Basic dropdown -->
                </div>
                ...
                <div class="d-fex">
                  ...
                  >
                    <div class="d-inline-block" style="font-size: 13px">
                      {{ viewsPagination.from }}-{{ viewsPagination.to }} /
                      {{ viewsPagination.total }}
                    </div>
                    <div class="d-inline-block">
                      <a
                        href="javascript:void(0)"
                        :style="{
                          'pointer-events':
                            viewsPagination.current_page == 1 ? 'none' : '',
                        }"
                        @click.prevent="
                          getData(viewsPagination.current_page - 1)
                        "
                      >
                        <span>&lt;</span>
                      </a>
                      ...
                      <a
                        ...
                        :style="{
                          'pointer-events':
                            viewsPagination.last_page ==
                            viewsPagination.current_page
                              ? 'none'
                              : '',
                        }"
                        @click.prevent="
                  ...
                  <tr>
                    ...
                    <th v-if="setting.name && isVisible('name')">
                      <div class="d-flex justify-content-center">
                        <span>{{ getCompanyKey("view_name_ar") }}</span>
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="views.sort(sortString('name'))"
                          ></i>
                          <i
                    ...
                    <th v-if="setting.name_e && isVisible('name_e')">
                      <div class="d-flex justify-content-center">
                        <span>{{ getCompanyKey("view_name_en") }}</span>
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="views.sort(sortString('name_e'))"
                          ></i>
                          <i
                            class="fas fa-arrow-down"
                            @click="views.sort(sortString('-name_e'))"
                  ...
                  >
                    ...
                    <td v-if="enabled3" class="do-not-print">
                      ...
                      <b-modal
                        ...
                        :title="getCompanyKey('view_edit_form')"
                        title-class="font-18"
                        body-class="p-4"
                        :ref="`edit-${data.id}`"
                        :hide-footer="true"
