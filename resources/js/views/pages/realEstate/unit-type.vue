...
export default {
  ...
  methods: {
    ...
    async getBranch() {
      ...
      await adminApi
        .get(`/real-estate/unit-type?company_id=${this.company_id}`)
        .then((res) => {
          let l = res.data.data;
        })
        .catch((err) => {
    ...
    log(id) {
      if (this.mouseEnter != id) {
        ...
        this.mouseEnter = id;
        adminApi
          .get(`/real-estate/unit-type/logs/${id}`)
          .then((res) => {
            let l = res.data.data;
            l.forEach((e) => {
    ...
    getData(page = 1) {
      ...
      adminApi
        .get(
          `/real-estate/unit-type?page=${page}&per_page=${this.per_page}&search=${this.search}&${filter}`
        )
        .then((res) => {
          let l = res.data;
          this.types = l.data;
          this.typesPagination = l.pagination;
    ...
    getDataCurrentPage(page = 1) {
      if (
        this.current_page <= this.typesPagination.last_page &&
        this.current_page != this.typesPagination.current_page &&
        this.current_page
      ) {
        this.isLoader = true;
        let filter = "";
      ...
      if (Array.isArray(id)) {
        Swal.fire({
          ...
          if (result.value) {
            this.isLoader = true;
            adminApi
              .post(`/real-estate/unit-type/bulk-delete`, { ids: id })
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            adminApi
              .delete(`/real-estate/unit-type/${id}`)
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
    ...
    AddSubmit() {
      ...
      if (this.$v.create.$invalid) {
        ...
        this.errors = {};
        adminApi
          .post(`/real-estate/unit-type`, this.create)
          .then((res) => {
    ...
    editSubmit(id) {
      ...
      if (this.$v.edit.$invalid) {
        ...
        this.errors = {};
        adminApi
          .put(`/real-estate/unit-type/${id}`, this.edit)
          .then((res) => {
     ...
     */
    resetModalEdit(id) {
      let branch = this.types.find((e) => id == e.id);
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row justify-content-between align-items-center mb-2">
              <h4 class="header-title">{{ $t("general.unitTypesTable") }}</h4>
                ...
                <div class="d-inline-block" style="width: 22.2%">
                  ...
                  >
                    <b-form-checkbox
                      ...
                      value="name"
                      class="mb-1"
                      >{{ getCompanyKey("unit_type_name_ar") }}</b-form-checkbox
                    >
                    <b-form-checkbox
                      ...
                      value="name_e"
                      class="mb-1"
                      >{{ getCompanyKey("unit_type_name_en") }}</b-form-checkbox
                    >
                  </b-dropdown>
                  <!-- Basic dropdown -->
                </div>
                ...
                <div class="d-fex">
                  ...
                  >
                    <div class="d-inline-block" style="font-size: 13px">
                      {{ typesPagination.from }}-{{ typesPagination.to }} /
                      {{ typesPagination.total }}
                    </div>
                    <div class="d-inline-block">
                      <a
                        href="javascript:void(0)"
                        :style="{
                          'pointer-events':
                            typesPagination.current_page == 1 ? 'none' : '',
                        }"
                        @click.prevent="
                          getData(typesPagination.current_page - 1)
                        "
                      >
                        <span>&lt;</span>
                      </a>
                      ...
                      <a
                        ...
                        :style="{
                          'pointer-events':
                            typesPagination.last_page ==
                            typesPagination.current_page
                              ? 'none'
                              : '',
                        }"
                        @click.prevent="
                  ...
                  <tr>
                    ...
                    <th v-if="setting.name && isVisible('name')">
                      <div class="d-flex justify-content-center">
                        <span>{{ getCompanyKey("unit_type_name_ar") }}</span>
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="types.sort(sortString('name'))"
                          ></i>
                          <i
                    ...
                    <th v-if="setting.name_e && isVisible('name_e')">
                      <div class="d-flex justify-content-center">
                        <span>{{ getCompanyKey("unit_type_name_en") }}</span>
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="types.sort(sortString('name_e'))"
                          ></i>
                          <i
                            class="fas fa-arrow-down"
                            @click="types.sort(sortString('-name_e'))"
                  ...
                  >
                    ...
                    <td v-if="enabled3" class="do-not-print">
                      ...
                      <b-modal
                        ...
                        :title="getCompanyKey('unit_type_edit_form')"
                        title-class="font-18"
                        body-class="p-4"
                        :ref="`edit-${data.id}`"
                        :hide-footer="true"
