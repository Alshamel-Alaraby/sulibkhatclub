...
export default {
  ...
  beforeRouteEnter(to, from, next) {
        next((vm) => {
      return permissionGuard(vm, "Finishing", "all Branch");
    });
  ...
  methods: {
    ...
    async getBranch() {
      ...
      await adminApi
        .get(`/real-estate/finishing?company_id=${this.company_id}`)
        .then((res) => {
          let l = res.data.data;
        })
        .catch((err) => {
    ...
    log(id) {
      if (this.mouseEnter != id) {
        ...
        this.mouseEnter = id;
        adminApi
          .get(`/real-estate/finishing/logs/${id}`)
          .then((res) => {
            let l = res.data.data;
            l.forEach((e) => {
    ...
    getData(page = 1) {
      ...
      adminApi
        .get(
          `/real-estate/finishing?page=${page}&per_page=${this.per_page}&search=${this.search}&${filter}`
        )
        .then((res) => {
          let l = res.data;
          this.finishing = l.data;
          this.finishingPagination = l.pagination;
    ...
    getCustomTableFields() {
      adminApi
        .get(`/customTable/table-columns/real-estate/rlst_finishings`)
        .then((res) => {
    ...
    getDataCurrentPage(page = 1) {
      if (
        this.current_page <= this.finishingPagination.last_page &&
        this.current_page != this.finishingPagination.current_page &&
        this.current_page
      ) {
        this.isLoader = true;
        let filter = "";
      ...
      if (Array.isArray(id)) {
        Swal.fire({
          ...
          if (result.value) {
            this.isLoader = true;
            adminApi
              .post(`/real-estate/finishing/bulk-delete`, { ids: id })
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            adminApi
              .delete(`/real-estate/finishing/${id}`)
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
    ...
    AddSubmit() {
      ...
      if (this.$v.create.$invalid) {
        ...
        this.errors = {};
        adminApi
          .post(`/real-estate/finishing`, {...this.create,company_id: this.$store.getters["auth/company_id"],})
          .then((res) => {
    ...
    editSubmit(id) {
      ...
      if (this.$v.edit.$invalid) {
        ...
        this.errors = {};
        adminApi
          .put(`/real-estate/finishing/${id}`, this.edit)
          .then((res) => {
     ...
     */
    resetModalEdit(id) {
      let branch = this.finishing.find((e) => id == e.id);
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row justify-content-between align-items-center mb-2">
              <h4 class="header-title">{{ $t("general.finishingTable") }}</h4>
                ...
                <div class="d-inline-block" style="width: 22.2%">
                  ...
                  >
                    <b-form-checkbox
                      ...
                      value="name"
                      class="mb-1"
                      >{{ getCompanyKey("finishing_name_ar") }}</b-form-checkbox
                    >
                    <b-form-checkbox
                      ...
                      value="name_e"
                      class="mb-1"
                      >{{ getCompanyKey("finishing_name_en") }}</b-form-checkbox
                    >
                  </b-dropdown>
                  <!-- Basic dropdown -->
                </div>
            ...
            >
              <div class="col-md-3 d-flex align-items-center mb-1 mb-xl-0">
                ...
                <b-button
                  v-b-modal.create
                  variant="primary"
                  v-if="isPermission('create Branch')"
                ...
                <div class="d-inline-flex">
                  ...
                  <button
                    ...
                    v-if="checkAll.length == 1 && isPermission('update Branch')"
                  >
                  ...
                  <button
                    class="custom-btn-dowonload"
                    v-if="checkAll.length > 1 && isPermission('delete Branch')"
                ...
                <div class="d-fex">
                  ...
                  >
                    <div class="d-inline-block" style="font-size: 13px">
                      {{ finishingPagination.from }}-{{ finishingPagination.to }} /
                      {{ finishingPagination.total }}
                    </div>
                    <div class="d-inline-block">
                      <a
                        href="javascript:void(0)"
                        :style="{
                          'pointer-events':
                            finishingPagination.current_page == 1 ? 'none' : '',
                        }"
                        @click.prevent="
                          getData(finishingPagination.current_page - 1)
                        "
                      >
                        <span>&lt;</span>
                      </a>
                      ...
                      <a
                        ...
                        :style="{
                          'pointer-events':
                            finishingPagination.last_page ==
                            finishingPagination.current_page
                              ? 'none'
                              : '',
                        }"
                        @click.prevent="
            ...
            <b-modal
              id="create"
              :title="getCompanyKey('finishing_create_form')"
              title-class="font-18"
              body-class="p-4 "
              :hide-footer="true"
              @show="resetModal"
            ...
            >
              ...
              >
                  ...
                  <tr>
                    ...
                    <th v-if="setting.name && isVisible('name')">
                      <div class="d-flex justify-content-center">
                        <span>{{ getCompanyKey("finishing_name_ar") }}</span>
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="finishing.sort(sortString('name'))"
                          ></i>
                          <i
                    ...
                    <th v-if="setting.name_e && isVisible('name_e')">
                      <div class="d-flex justify-content-center">
                        <span>{{ getCompanyKey("finishing_name_en") }}</span>
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="finishing.sort(sortString('name_e'))"
                          ></i>
                          <i
                            class="fas fa-arrow-down"
                            @click="finishing.sort(sortString('-name_e'))"
                ...
                <tbody v-if="finishing.length > 0">
                  <tr
                    @click.capture="checkRow(data.id)"
                    @dblclick.prevent="
                      isPermission('update Branch')
                        ? $bvModal.show(`modal-edit-${data.id}`)
                        : false
                    "
                    v-for="(data, index) in finishing"
                    :key="data.id"
                  ...
                  >
                    ...
                    <td v-if="enabled3" class="do-not-print">
                      <div class="btn-group">
                        ...
                        <div class="dropdown-menu dropdown-menu-custom">
                          <a
                            class="dropdown-item"
                            href="#"
                            v-if="isPermission('update Branch')"
                          ...
                          <a
                            ...
                            href="#"
                            v-if="isPermission('delete Branch')"
                      ...
                      <b-modal
                        ...
                        :title="getCompanyKey('finishing_edit_form')"
                        title-class="font-18"
                        body-class="p-4"
                        :ref="`edit-${data.id}`"
                        :hide-footer="true"
