...
export default {
    ...
    methods: {
        ...
        getData(page = 1) {
            ...
            adminApi
                .get(
                    `/streets?page=${page}&per_page=${this.per_page}&search=${this.search}&${filter}`
                )
                .then((res) => {
                    let l = res.data;
                    this.streets = l.data;
                    this.streetsPagination = l.pagination;
        ...
        getDataCurrentPage(page = 1) {
            if (
                this.current_page <= this.streetsPagination.last_page &&
                this.current_page != this.streetsPagination.current_page &&
                this.current_page
            ) {
                this.isLoader = true;
                let filter = "";
                ...
                adminApi
                    .get(
                        `/streets?page=${this.current_page}&per_page=${this.per_page}&search=${this.search}&${filter}`
                    )
                    .then((res) => {
                        let l = res.data;
                        this.streets = l.data;
            ...
            if (Array.isArray(id)) {
                Swal.fire({
                    ...
                    if (result.value) {
                        this.isLoader = true;
                        adminApi
                            .post(`/streets/bulk-delete`, { ids: id })
                            .then((res) => {
                                this.checkAll = [];
                                this.getData();
                                Swal.fire({
        ...
        async resetModalEdit(id) {
            if (this.isVisible('avenue_id')) await this.getAvenue();
            let avenues = this.streets.find((e) => id == e.id);
            this.edit.name = avenues.name;
            this.edit.name_e = avenues.name_e;
            this.edit.is_active = avenues.is_active;
            this.edit.avenue_id = avenues.avenue_id ?? null;
            this.errors = {};
        },
        /**
         *  hidden Modal (edit)
        ...
        ExportExcel(type, fn, dl) {
            ...
            setTimeout(() => {
                ...
                if (dl) {
                    ...
                    XLSX.writeFile(wb, fn || (('streets' + '.' || 'SheetJSTableExport.') + (type || 'xlsx')));
                }
                this.enabled3 = true;
            }, 100);
        },
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row justify-content-between align-items-center mb-2">
                            <h4 class="header-title">{{ $t("general.streetsTable") }}</h4>
                                ...
                                <div class="d-inline-block" style="width: 22.2%">
                                    ...
                                    <b-dropdown variant="primary" :text="$t('general.searchSetting')" ref="dropdown"
                                        ...
                                        <b-form-checkbox v-if="isVisible('name_e')" v-model="filterSetting" value="name_e" class="mb-1">{{
                                            getCompanyKey("street_name_en") }}</b-form-checkbox>
                                        <b-form-checkbox v-if="isVisible('avenue_id')" v-model="filterSetting"
                                                         :value="$i18n.locale == 'ar' ? 'avenue.name' : 'avenue.name_e'" class="mb-1">{{ getCompanyKey("street_avenue")
                                            }}</b-form-checkbox>
                                    </b-dropdown>
                                    <!-- Basic dropdown -->
                                </div>
                  ...
                  <span :class="[
                    ...
                    <i class="fe-search"></i>
                                ...
                                <div class="d-fex">
                                    ...
                                    <div class="d-inline-block">
                                        ...
                                        <b-dropdown variant="primary" :html="`${$t('general.setting')} <i class='fe-settings'></i>`"
                                            ...
                                            <b-form-checkbox v-if="isVisible('avenue_id')" v-model="setting.avenue_id" class="mb-1">
                                    ...
                                    <div class="d-inline-flex align-items-center pagination-custom">
                                        <div class="d-inline-block" style="font-size: 13px">
                                            {{ streetsPagination.from }}-{{ streetsPagination.to }} /
                                            {{ streetsPagination.total }}
                                        </div>
                                        <div class="d-inline-block">
                                            <a href="javascript:void(0)" :style="{
                        'pointer-events':
                          streetsPagination.current_page == 1 ? 'none' : '',
                      }" @click.prevent="getData(streetsPagination.current_page - 1)">
                                                <span>&lt;</span>
                                            </a>
                                            ...
                                            <a href="javascript:void(0)" :style="{
                        'pointer-events':
                          streetsPagination.last_page == streetsPagination.current_page
                            ? 'none'
                            : '',
                      }" @click.prevent="getData(streetsPagination.current_page + 1)">
                                                <span>&gt;</span>
                        ...
                        <!--  create   -->
                        <b-modal id="create" :title="getCompanyKey('street_create_form')" title-class="font-18" body-class="p-4 "
                        ...
                        <div class="table-responsive mb-3 custom-table-theme position-relative">
                            ...
                            <table class="table table-borderless table-hover table-centered m-0" ref="exportable_table" id="printData">
                                ...
                                <tr>
                                    ...
                                    <th v-if="setting.name && isVisible('name')">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ getCompanyKey("street_name_ar") }}</span>
                                            <div class="arrow-sort">
                                                <i class="fas fa-arrow-up" @click="streets.sort(sortString('name'))"></i>
                                                <i class="fas fa-arrow-down" @click="streets.sort(sortString('-name'))"></i>
                                            </div>
                                        </div>
                                    </th>
                                    <th v-if="setting.name_e && isVisible('name_e')">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ getCompanyKey("street_name_en") }}</span>
                                            <div class="arrow-sort">
                                                <i class="fas fa-arrow-up" @click="streets.sort(sortString('name_e'))"></i>
                                                <i class="fas fa-arrow-down" @click="streets.sort(sortString('-name_e'))"></i>
                                            </div>
                                        </div>
                                    </th>
                                    <th v-if="setting.avenue_id && isVisible('avenue_id')">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ getCompanyKey("street_avenue") }}</span>
                                        </div>
                                    </th>
                                    <th v-if="setting.is_active && isVisible('is_active')">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ getCompanyKey("street_status") }}</span>
                                            <div class="arrow-sort">
                                                <i class="fas fa-arrow-up" @click="streets.sort(sortString('name'))"></i>
                                                <i class="fas fa-arrow-down" @click="streets.sort(sortString('-name'))"></i>
                                            </div>
                                ...
                                <tr @click.capture="checkRow(data.id)" @dblclick.prevent="$bvModal.show(`modal-edit-${data.id}`)"
                                    v-for="(data, index) in streets" :key="data.id" class="body-tr-custom">
                                    ...
                                    <td v-if="setting.avenue_id && isVisible('avenue_id')">
                        ...
                        {{
                          ...
                          : `${$t("general.Inactive")}`
                                    ...
                                    <td v-if="enabled3" class="do-not-print">
                                        ...
                                        <!--  edit   -->
                                        <b-modal :id="`modal-edit-${data.id}`" :title="getCompanyKey('street_edit_form')"
                                                ...
                                                <div class="row">
                                                    <div class="col-md-12" v-if="isVisible('avenue_id')">
                                                        <div class="form-group position-relative">
                                                            ...
                                                            <multiselect @input="showBranchModalEdit" v-model="edit.avenue_id"
