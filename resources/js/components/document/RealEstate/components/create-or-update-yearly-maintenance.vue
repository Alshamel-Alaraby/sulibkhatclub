...
import PrintInvoice from "../print/print-general-real-estate";
import transactionBreak from "../../../create/receivablePayment/transactionBreak/transactionBreak";
import Guest from "../../../create/booking/guest";
 ...
 */
export default {
    name: "createOrUpdateYearlyMaintenanceContract",
    props: {
        document: {
            default: null,
        },
    ...
    data() {
        return {
            ...
            create: {
                ...
                header_details: [{
                    unit_id:null, //for units
                    building_id:null, //for building
                    quantity: 1,
                    price_per_uint: 0,
                    total: 0,
                    unit_type: "real_estate",
    ...
    methods: {
        ...
        getCustomers(customer_handel,employee_id=null,search='') {
            this.isLoader = true;
            adminApi
                .get(`/general-customer?limet=10&company_id=${this.company_id}&type=1&employee_id=${customer_handel=='his_customer'?employee_id:''}&search=${search}&columns[0]=name&columns[1]=name_e&columns[2]=id`)
                .then((res) => {
                    this.isLoader = false;
                    let l = res.data.data;
                        l.unshift({ id: 0, name: "اضف نزيل", name_e: "Add Guest" });
                    this.customers = l;
        ...
        showUnits(index){
          if (this.create.header_details[index].building_id)
          {
              this.create.header_details[index].unit_id=null;
              this.getUnites(index);
          }else {
              this.create.header_details[index].unit_id=null;
              this.units = [];
          }
        ...
        getUnites(index){
            this.isLoader = true;
            adminApi
                .get(`/real-estate/units?building_id=${this.create.header_details[index].building_id}`)
                .then((res) => {
                    let l = res.data.data;
                    this.units = l;
                })
        ...
        getDiffDays(index)
        {
            ...
            this.create.header_details[index].rent_days = diffDaysPlus -1;
        },
        showBreakCreate(){
            if (this.create.id) {
        ...
        showGuestModal() {
            if (this.create.customer_id == 0) {
                this.$bvModal.show(this.guestModelId);
    ...
    <div>
        <div style="display:none;">
            <Guest
                ...
                :id="guestModelId" @created="getCustomers(salesmen.find( (el) => el.id == create.employee_id ).customer_handel,create.employee_id)"
            />
        </div>
                                    ...
                                    <div class="col-12 col-lg-12">
                                        ...
                                        <div>
                                            <div class="row text-600 text-white bgc-default-tp1 py-25">
                                                <div class="col-2">{{ $t('general.documentBuilding') }}</div>
                                                <div class="col-2">{{ $t('general.Unit') }}</div>
                                                <div class="col-2">{{ $t('general.from_date') }}</div>
                                                <div class="col-2">{{ $t('general.to_date') }}</div>
                                                <div class="col-2">{{ $t('general.amount') }}</div>
                                            ...
                                            <div v-for="(item, gIndex) in create.header_details" class="text-95  text-secondary-d3">
                                                <div class="row mb-2 mb-sm-0 py-25" :class="[gIndex % 2 == 0 ? 'bgc-default-l4' : '' ]" >
                                                    <div class="col-2 p-0">
                                                        <multiselect
                                                            :show-labels="false"
                                                            @input="showUnits(gIndex)"
                                                            v-model="$v.create.header_details.$each[gIndex].building_id.$model"
                                                            ...
                                                            :class="{'is-invalid':$v.create.header_details.$each[gIndex].building_id.$error || errors.building_id,}"
                                                        >
                                                        </multiselect>
                                                    </div>
                                                    <div class="col-2 p-0">
                                                        <multiselect
                                                            ...
                                                            :custom-label="(opt) => units.find((x) => x.id == opt) ? $i18n.locale == 'ar'? units.find((x) => x.id == opt).name : units.find((x) => x.id == opt).name_e :''"
