...
import PrintInvoice from "./print/print-hms-invoice";
...
export default {
    ...
    data() {
        return {
            ...
            setting: {
                ...
                from_doctor_id: true,
                branch_id: true,
                total_patient_amount: true,
                total_company_insurance_amount: true,
                patient_insurance_number: true,
                doctor_amount: true,
                bill_holder: true,
                serial_id: true,
            },
            filterSetting: [
                ...
                'total_company_insurance_amount',
    ...
    methods: {
        printModal(doc){
            this.documentDataForPrint = doc
            this.$bvModal.show('PrintGeneralInvoice');
        ...
        getData(page = 1) {
            ...
            let index = this.filterSetting.indexOf("patient_id");
            if (index > -1) {
                _filterSetting[index] =
                    this.$i18n.locale == "ar" ? "patient.name" : "patient.name_e";
            }
            index = this.filterSetting.indexOf("doctor_id");
            if (index > -1) {
                _filterSetting[index] =
                    this.$i18n.locale == "ar" ? "doctor.name" : "doctor.name_e";
            }

            index = this.filterSetting.indexOf("from_doctor_id");
            if (index > -1) {
                _filterSetting[index] =
                    this.$i18n.locale == "ar" ? "from_doctor.name" : "from_doctor.name_e";
            }
            index = this.filterSetting.indexOf("company_insurance_id");
            if (index > -1) {
                _filterSetting[index] =
                    this.$i18n.locale == "ar" ? "company_insurance.name" : "company_insurance.name_e";
            }
        ...
        getDataCurrentPage() {
            if (
                ...
                index = this.filterSetting.indexOf("doctore_id");
                if (index > -1) {
                    _filterSetting[index] =
                        this.$i18n.locale == "ar" ? "doctore.name" : "doctore.name_e";
                }
...
<template>
    <div>
        ...
        <template v-if="document && document.document_detail_type == 'normal' && (isPermission('create HMS Invoice') || isPermission('update HMS Invoice'))">
            <CreateOrUpdateItems :bill_holder="''" :appointment_data="{}" :companyKeys="companyKeys" :dataRow="selected_data_row" :defaultsKeys="defaultsKeys" :document="document" :paymentMethods="paymentMethods" :companies_insurances="companies_insurances"
             :document_id="document_id"  @created="getData()" :doctors="doctors"  :services="services"/>
        </template>
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                            ...
                            <div class="col-xs-10 col-md-9 col-lg-7" style="font-weight: 500">
                                <div class="d-inline-block" style="width: 22.2%">
                                    ...
                                    <b-dropdown variant="primary" :text="$t('general.searchSetting')" ref="dropdown"
                                        ...
                                        <b-form-checkbox v-model="filterSetting" value="patient_id" class="mb-1">{{ $t('general.Patient') }}</b-form-checkbox>
                                        <b-form-checkbox v-model="filterSetting" value="doctor_id" class="mb-1">{{  $t('general.Doctor')}}</b-form-checkbox>
                                        <b-form-checkbox v-model="filterSetting" value="doctor_id" class="mb-1">{{  $t('general.From Doctor')}}</b-form-checkbox>
                                        <b-form-checkbox v-model="filterSetting" value="company_insurance_id" class="mb-1">{{  $t('general.Company Insurance')}}</b-form-checkbox>
                        ...
                        <div class="row justify-content-between align-items-center mb-2 px-1">
                            <div class="col-md-3 d-flex align-items-center mb-1 mb-xl-0">
                                <b-button @click.prevent="selected_data_row = {};$bvModal.show(`createInvoice`)" variant="primary" class="btn-sm mx-1 font-weight-bold" v-if="isPermission('create HMS Invoice')">
                                    {{ $t("general.Create") }}
                                    <i class="fas fa-plus"></i>
                                </b-button>
                                <div class="d-inline-flex">
                                    ...
                                    <!-- end mult delete  -->
                                    <!--  start one delete  -->
                                    <button class="custom-btn-dowonload" v-if="checkAll.length == 1 &&  isPermission('delete HMS Invoice')"
                            ...
                            <div class="col-xs-10 col-md-9 col-lg-7 d-flex align-items-center justify-content-end">
                                <div>
                                    ...
                                    <b-dropdown variant="primary"
                                        ...
                                        <b-form-checkbox v-model="setting.patient_id" value="patient_id" class="mb-1">{{ $t('general.Patient') }}</b-form-checkbox>
                                        <b-form-checkbox v-model="setting.doctor_id" value="doctor_id" class="mb-1">{{  $t('general.Doctor')}}</b-form-checkbox>
                                        <b-form-checkbox v-model="setting.doctor_id" value="doctor_id" class="mb-1">{{  $t('general.From Doctor')}}</b-form-checkbox>
                                        <b-form-checkbox v-model="setting.company_insurance_id" value="company_insurance_id" class="mb-1">{{  $t('general.Company Insurance')}}</b-form-checkbox>
                        ...
                        <div class="table-responsive mb-3 custom-table-theme position-relative">
                            ...
                            <table class="table table-borderless table-hover table-centered m-0" ref="exportable_table" id="printReservation">
                                <thead>
                                <tr>
                                    <th scope="col" style="width: 0" v-if="enabled3 &&  isPermission('delete HMS Invoice')" class="do-not-print">
                                    ...
                                    <th v-if="setting.patient_id">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ $t('general.Patient') }}</span>

                                        </div>
                                    </th>
                                    <th v-if="setting.doctor_id">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ $t('general.Doctor')}}</span>

                                        </div>
                                    </th>
                                    <th v-if="setting.from_doctor_id">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ $t('general.From Doctor')}}</span>

                                        </div>
                                    </th>
                                    <th v-if="setting.bill_holder">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ $t('general.Bill holder')}}</span>

                                        </div>
                                    </th>
                                    <th v-if="setting.company_insurance_id">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ $t('general.Company Insurance')}}</span>

                                        </div>
                                    </th>
                                    <th v-if="setting.patient_insurance_number">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ $t('general.Patient insurance number')}}</span>
                                            <div class="arrow-sort">
                                                <i class="fas fa-arrow-up" @click="invoices.sort(sortString('patient_insurance_number'))"></i>
                                                <i class="fas fa-arrow-down" @click="invoices.sort(sortString('-patient_insurance_number'))"></i>
                                            </div>
                                        </div>
                                    </th>
                                    <th v-if="setting.total_patient_amount">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ $t('general.Patient Amount')}}</span>
                                            <div class="arrow-sort">
                                                <i class="fas fa-arrow-up" @click="invoices.sort(sortString('total_patient_amount'))"></i>
                                                <i class="fas fa-arrow-down" @click="invoices.sort(sortString('-total_patient_amount'))"></i>
                                            </div>
                                        </div>
                                    </th>
                                    <th v-if="setting.total_company_insurance_amount">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ $t('general.Company Amount')}}</span>
                                            <div class="arrow-sort">
                                                <i class="fas fa-arrow-up" @click="invoices.sort(sortString('total_company_insurance_amount'))"></i>
                                                <i class="fas fa-arrow-down" @click="invoices.sort(sortString('-total_company_insurance_amount'))"></i>
                                            </div>
                                        </div>
                                    </th>
                                    <th v-if="setting.doctor_amount">
                                        <div class="d-flex justify-content-center">
                                            <span>{{ $t('general.Doctor Amount')}}</span>
                                            <div class="arrow-sort">
                                                <i class="fas fa-arrow-up" @click="invoices.sort(sortString('doctor_amount'))"></i>
                                                <i class="fas fa-arrow-down" @click="invoices.sort(sortString('-doctor_amount'))"></i>
                                            </div>
                                        </div>
                                    </th>
                                    ...
                                    <th v-if="enabled3 && (isPermission('update HMS Invoice') || isPermission('delete HMS Invoice'))" class="do-not-print">
                                        {{ $t("general.Action") }}
                                    </th>
                                ...
                                <tr @click.capture="checkRow(data.id)"
                                    @dblclick.prevent="selected_data_row = data ;$bvModal.show(`createInvoice`);"
                                    v-for="(data, index) in invoices" :key="data.id" class="body-tr-custom">
                                    <td v-if="enabled3 &&  isPermission('delete HMS Invoice')" class="do-not-print">
                                    ...
                                    <td v-if="setting.patient_id">
                                        <h5 class="m-0 font-weight-normal">
                                            {{$i18n.locale == "ar" ? data.patient.name : data.patient.name_e}}
                                        </h5>
                                    </td>
                                    <td v-if="setting.doctor_id">
                                        <h5 class="m-0 font-weight-normal">
                                            {{ $i18n.locale == "ar" ? data.doctor.name : data.doctor.name_e }}
                                        </h5>
                                    </td>
                                    <td v-if="setting.from_doctor_id">
                                        <h5 class="m-0 font-weight-normal">
                                            {{data.from_doctor?( $i18n.locale == "ar" ? data.from_doctor.name : data.from_doctor.name_e) :'-' }}
                                        </h5>
                                    </td>
                                    <td v-if="setting.bill_holder">
                                        <h5 class="m-0 font-weight-normal">
                                            {{ setting.total_patient_amount > 0 || data.total_company_insurance_amount > 0 ? (
                                                data.total_patient_amount > 0 ? $t('general.Patient') + (data.total_company_insurance_amount > 0 ? " / "+$t('general.Company Insurance') : ''): (data.total_company_insurance_amount > 0 ? $t('general.Company Insurance') : '')
                                            ) : $t('general.Doctor') }}
                                        </h5>
                                    </td>
                                    <td v-if="setting.company_insurance_id">
                                        <h5 class="m-0 font-weight-normal">
                                            {{ $i18n.locale == "ar" ? data.company_insurance.name : data.company_insurance.name_e }}
                                        </h5>
                                    </td>
                                    <td v-if="setting.patient_insurance_number">
                                        <h5 class="m-0 font-weight-normal">
                                            {{ data.patient_insurance_number??'-' }}
                                        </h5>
                                    </td>
                                    <td v-if="setting.total_patient_amount">
                                        <h5 class="m-0 font-weight-normal">
                                            {{ data.total_patient_amount ?? '-' }}
                                        </h5>
                                    </td>
                                    <td v-if="setting.total_company_insurance_amount">
                                        <h5 class="m-0 font-weight-normal">
                                            {{ data.total_company_insurance_amount?? '-' }}
                                        </h5>
                                    </td>
                                    <td v-if="setting.doctor_amount">
                                        <h5 class="m-0 font-weight-normal">
                                            {{ data.total_company_insurance_amount == 0 && data.total_patient_amount == 0? data.net_invoice : '-' }}
                                        </h5>
                                    </td>
                                    ...
                                    <td v-if="enabled3 && (isPermission('update HMS Invoice') || isPermission('delete HMS Invoice'))" class="do-not-print">
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-custom">
                                                <!-- <a class="dropdown-item" href="#" v-if="isPermission('update HMS Invoice')"
                                                   @click="selected_data_row = data ;$bvModal.show(`createInvoice`);">
                                                    <div
                                                ...
                                                <a v-if="document && data.id && document_id != 34"
                                                   class="dropdown-item" href="#" @click="printModal(data)">
                                                ...
                                                <a class="dropdown-item text-black" href="#" v-if="isPermission('delete HMS Invoice')"
        ...
        <div style="display:none;">
            <template v-if="document && document_id != 34">
                <PrintInvoice :id="'PrintGeneralInvoice'" :document_row_id="documentDataForPrint.id" :documentDataForPrint="documentDataForPrint"/>
            </template>
        </div>
    </div>
</template>
