<template>
    <div>
        ...
        <b-modal
            ...
            :title="
        type != 'edit'
          ? getCompanyKey('department_create_form')
          : getCompanyKey('department_edit_form')
      "
            title-class="font-18"
            body-class="p-4"
            size="xll"
        ...
        >
            <form>
                ...
                <b-tabs nav-class="nav-tabs nav-bordered">
                    <b-tab :title="$t('general.DataEntry')" active>
                        ...
                        'is-valid':
                          !$v.create.name_e.$invalid && !errors.name_e,
                            ...
                            <button class="btn col-2 btn-sm btn-primary h-50 mt-4" @click.prevent="addNewField" v-if="isVisible('locations')">
                                {{ $t('general.Add') }} <i class="fa fa-plus"></i></button>
                        </div>
                        <template v-if="isVisible('locations')">
                            <div class="row" v-for="(item, index) in create.locations" :key="index">

                                <div class="col-md-3 position-relative">
                                    <div class="form-group">
                                        <label class="my-1 mr-2">{{ getCompanyKey('locations') }} Locations</label>
                                        <span class="text-danger">*</span>
                                        <multiselect
                                            v-model="$v.create.locations.$each[index].location_id.$model"
                                            :options="locations.map((type) => type.id)"
                                            :custom-label="(opt) => locations.find((x) => x.id ==opt)
                                            ? $i18n.locale == 'ar'
                                            ? locations.find((x) => x.id == opt).name
                                            : locations.find((x) => x.id == opt).name_e
                                            : ''
                                            "
                                            :class="{ 'is-invalid': $v.create.locations.$each[index].location_id.$error || errors[`location.${index}.location_id`] }">

                                        </multiselect>
                                        <template v-if="errors && errors[`locations.${index}.location_id`]">
                                            <ErrorMessage
                                                v-for="(errorMessage, index) in errors[`locations.${index}.location_id`]"
                                                :key="index">{{
                                                    errorMessage
                                                }}
                                            </ErrorMessage>
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label class="my-1 mr-2">
                                            {{ getCompanyKey("department_task_supervisors") }}
                                            <span v-if="isRequired('locations')" class="text-danger"
                                            >*</span
                                            >
                                        </label>
                                        <multiselect
                                            :multiple="true"
                                            v-model="$v.create.locations.$each[index].supervisors.$model"
                                        ...
                                        <template v-if="errors && errors[`locations.${index}.supervisors`]">
                                            <ErrorMessage
                                                v-for="(errorMessage, index) in errors[`locations.${index}.supervisors`]"
                                                :key="index">{{
                                                    errorMessage
                                                }}
                                            </ErrorMessage>
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <multiselect
                                            :multiple="true"
                                            v-model="$v.create.locations.$each[index].attentions.$model"
                                            :options="employees.map((type) => type.id)"
                                            :custom-label="
                                            (opt) =>
                                                employees.find((x) => x.id == opt)
                                        ...
                                        <template v-if="errors && errors[`locations.${index}.attentions`]">
                                            <ErrorMessage
                                                v-for="(errorMessage, index) in errors[`locations.${index}.attentions`]"
                                                :key="index">{{
                                                    errorMessage
                                                }}
                                            </ErrorMessage>
                                ...
                                <div class="col-md-2" >
                                    <div class="form-group">
                                        <label class="my-1 mr-2">
                                            {{ getCompanyKey("department_engineers") }}
                                            <span v-if="isRequired('locations')" class="text-danger"
                                            >*</span
                                            >
                                        </label>
                                        <multiselect
                                            :multiple="true"
                                            v-model="$v.create.locations.$each[index].engineers.$model"
                                            :options="employees.map((type) => type.id)"
                                            :custom-label="
                                            (opt) =>
                                                employees.find((x) => x.id == opt)
                                        ...
                                        <template v-if="errors && errors[`locations.${index}.engineers`]">
                                            <ErrorMessage
                                                v-for="(errorMessage, index) in errors[`locations.${index}.engineers`]"
                                                :key="index">{{
                                                    errorMessage
                                                }}
                                            </ErrorMessage>
                                ...
                                <div class="col-md-1 pt-3">
                                    <button v-if="create.locations.length > 1" type="button"
                    ...
                    <b-tab :disabled="!department_id" :title="$t('general.tasks')">
                                        ...
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-custom">
                                                ...
                                                <a
                                                    ...
                                                    href="#"
                                                    @click.prevent="deleteTasks(data2.id)"
                                                >
                                                    <div
                                                        class="d-flex justify-content-between align-items-center text-black"
                                                    >
        ...
        <b-modal
            ...
            :title="
        typeTask
          ? getCompanyKey('department_task_create_form')
          : getCompanyKey('department_task_edit_form')
      "
            title-class="font-18"
            body-class="p-4 "
            size="lg"
        ...
        >
            <form>
                ...
                <div class="row">
                    ...
                    'is-valid':
                      !$v.createTask.name_e.$invalid && !errors.name_e,
                  }"
                                />
                            </div>
                            <div
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <textarea
                                @input="arabicValueDescriptionTask(createTask.description)"
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <textarea
                                @input="englishValueDescriptionTask(createTask.description_e)"
...
export default {
        ...
        create: {
            ...
            locations: {
                ...
                $each: {
                    ...
                    engineers: {
                        required: requiredIf(function (model) {
                            return this.isRequired("engineers");
                        }),
                    },
                    supervisors: {
                        required: requiredIf(function (model) {
    ...
    methods: {
        removeNewField(index) {
            if (this.create.locations.length > 1) {
                this.create.locations.splice(index, 1);
            }
        },

        addNewField() {
            this.create.locations.push({
                location_id: null,
                supervisors: [],
                attentions: [],
                engineers: [],
        ...
        resetModal() {
            ...
            setTimeout(async () => {
                if (this.type != "edit") {
                    ...
                    if (this.isVisible("supervisors") || this.isVisible("attentions") || this.isVisible("engineers"))
                        this.getEmployees();
                    this.getLocations();
                } else {
                    if (this.idObjEdit.dataObj) {
                        ...
                        module.locations.forEach((e, index) => {
                            this.create.locations.push({
