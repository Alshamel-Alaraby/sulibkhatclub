...
<div>
    ...
    <div>
        <Owner
            :companyKeys="companyKeys" :defaultsKeys="defaultsKeys" @created="getOwner"
            :id="'owner-create-wallet'" :isPage="false" type="create" :isPermission="isPermission"
        />
        <Building
            :companyKeys="companyKeys" :defaultsKeys="defaultsKeys" @created="getBuildings"
            :id="'building-create-owner'" :isPage="false" type="create" :isPermission="isPermission"
        />
        <!--  create   -->
        <b-modal
            :id="id"
            :title="type != 'edit'?getCompanyKey('wallet_create_form'):getCompanyKey('wallet_create_form')"
            title-class="font-18"
            body-class="p-4 "
                                    ...
                                    <div class="form-group">
                                        ...
                                        <multiselect @input="showOwnerModal(index)"
                                                     ...
                                                     :options="owners.map((type) => type.id)" :custom-label="
                                                                (opt) => owners.find((x) => x.id == opt)?
                                                                    $i18n.locale == 'ar'
                                                                        ? owners.find((x) => x.id == opt).name
                                                                        : owners.find((x) => x.id == opt).name_e: null
                                                            ">
                                        </multiselect>
            ...
            setTimeout( async () => {
                if(this.type != 'edit'){
                    ...
                    if(this.idObjEdit.dataObj){
                        ...
                        this.createBuilding.building_id = module.buildings.length ? module.buildings[0].id : null;
                        adminApi
                            .get(`/real-estate/wallets/bu-ty/${this.wallet_id}/${this.createBuilding.building_id}`)
                            .then((res) => {
                                this.createBuilding.bu_ty = res.data.data.bu_ty == 1 ? "inactive" : "active";
                            })
                            .catch((err) => {
                                Swal.fire({
                                    icon: "error",
        ...
        AddSubmitOwber() {
            ...
            if (this.$v.createOwner.$invalid && !this.is_persentage) {
                ...
                if(this.type != 'edit'){
                    ...
                    adminApi
                        .put(`/real-estate/wallet-owner/${this.wallet_id}`, { 'wallet-owner': this.createOwner.wallet_owners })
                        .then((res) => {
                            if(!this.isPage)
                                return;
                            else
