...
export default {
    ...
    props: {
        ...
        isPermission: {},
        url: { default: "/club-members/sponsers" },
    },
    data() {
        return {
            groups: [],
    ...
    validations: {
        create: {
            ...
            cm_members_id: {
                required: requiredIf(function (model) {
                    return this.isRequired("cm_members_id");
                }),
            },
        },
    },
    ...
    methods: {
        ...
        resetModal() {
            ...
            setTimeout(async () => {
                if (this.type != "edit") {
                    ...
                    if (this.isVisible("cm_members_id")) this.getMember();
                } else {
                    if (this.idObjEdit.dataObj) {
                        ...
                        this.create.cm_members_id = module.cm_members_id;
                    }
                }
            }, 50);
        },
        ...
        async getMember(search = "") {
            ...
            await adminApi
                .get(
                    `/club-members/member-requests/member-names?active=${1}&search=${search}
                `
                )
                .then((res) => {
        ...
        getMemberName() {
            if (this.create.cm_members_id) {
                const member = this.members.find(
                    (row) => this.create.cm_members_id == row.id
                );
                this.create.name = member ? member.full_name : null;
...
<template>
    ...
    <b-modal
        ...
        :title="
            type != 'edit'
                ? getCompanyKey('sponsor_create_form')
                : getCompanyKey('sponsor_edit_form')
        "
        title-class="font-18"
        body-class="p-4 "
        :hide-footer="true"
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group position-relative">
                        ...
                        <multiselect
                            ...
                            v-model="create.cm_members_id"
                            :options="members.map((type) => type.id)"
                            :custom-label="
                                (opt) =>
                                    members.find((x) => x.id == opt).full_name
                            "
                        >
                        </multiselect>
                        <template v-if="errors.cm_members_id">
                            <ErrorMessage
                                v-for="(
                                    errorMessage, index
                                ) in errors.cm_members_id"
                                :key="index"
                                >{{ errorMessage }}
                            </ErrorMessage>
                        </template>
                ...
                <div class="col-12" v-if="isVisible('group_id')">
                    <div class="form-group position-relative">
                        ...
                        <div
                            v-if="$v.create.group_id.$error || errors.group_id"
                            class="text-danger"
                        >
